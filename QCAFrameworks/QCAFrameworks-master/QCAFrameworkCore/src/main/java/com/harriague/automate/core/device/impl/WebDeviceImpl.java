...
public class WebDeviceImpl extends DeviceBase implements Device {
     ...
     */
    public WebDeviceImpl(Utils utils, String name) {
        this.utils = utils;
        this.name = name;
    }
    ...
    private static void killProcess(List<String> allProcessName) throws Exception {
        Utils OSUtil = CoreUtils.getOSUtil();
    ...
    public Agent createAgent(AppType type, String agentImplPackageTemplate) throws DeviceException {
        String platform;
        try {
            platform = ReadProperty.getProperty(PropertiesKeys.DEFAULT_PLATFORM);
